...
def verify_stock_via_naver(stock_name):
    try:
        search_url = f"https://finance.naver.com/search/searchList.naver?query={requests.utils.quote(stock_name)}"
    ...
    try:
        ...
        for _, row in df.iterrows():
            try:
                ...
                results.append({
                    "date": date_str, "open": open_p, "high": high,
...
def generate_candlestick_base64(daily_data, stock_name):
    try:
        ...
        style = mpf.make_mpf_style(
            marketcolors=mc, facecolor="#141420",
            edgecolor="#2a2a3e", gridcolor="#1e1e2e", gridstyle="--",
            rc={
                "axes.labelcolor": "#888", "xtick.color": "#666",
                "ytick.color": "#666", "font.size": 8,
            },
        )
        buf = io.BytesIO()
        mpf.plot(
            df, type="candle", style=style, volume=True,
            title=f"\n{stock_name}", ylabel="", ylabel_lower="",
            figsize=(5, 2.8), tight_layout=True,
            savefig=dict(
                fname=buf, dpi=130, bbox_inches="tight",
                facecolor="#141420", edgecolor="none",
            ),
        )
        buf.seek(0)
...
def analyze_and_generate_html(all_data, api_key, channels_data=None, gh_repo=""):
    ...
    for stype, items in grouped.items():
        ...
        for item in items[:50]:
            ...
            link = item.get("link", "")
            data_sections += f"- [{source}] {title}"
            if summary:
                data_sections += f" | {summary}"
            if link:
                data_sections += f" | URL: {link}"
            data_sections += "\n"
...
def generate_html(data, channels_data=None, gh_repo=""):
    ...
    for stock in hidden_picks:
        ...
        hidden_html += ('\n        <div class="stock-card hidden-card">'
            + '\n            <div class="stock-header">'
    ...
    if channels_data is not None:
        ...
        settings_html = ('\n    <div id="settings-panel" style="display:none; padding:20px; background:#1a1a2e; border-radius:12px; margin-top:20px;">'
