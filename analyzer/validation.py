...
def validate_stocks(data, api_key, all_data=None):
    ...
    def try_resolve_vague(text, context_stock_name, all_src_data, reason=None):
        ...
        skip_words = {"시장", "투자", "매수", "매도", "상승", "하락", "분석", "전망", "오늘", "내일", "이번", "다음", "최근", "올해", "내년", "지금", "종목", "주식", "기업", "회사", "업종", "섹터", "테마", "원전", "반도체", "방산", "바이오", "배터리", "수소", "로봇", "긴급", "속보", "단독", "추천", "공개", "비밀", "무료", "확인", "가능", "전문", "대표", "관련", "수익", "실적", "성장", "하반기", "상반기", "대비", "이상", "이하", "목표", "예상", "전략", "포트", "리스크", "기대", "우려", "가격", "거래", "물량", "수급", "외국인", "기관", "개인", "코스피", "코스닥"}
        seen = set()
        verified_names = []
    ...
    for hp in data.get("hidden_picks", []):
        ...
        for reason in hp.get("reasons", []):
            ...
            if fixed:
                reason["detail"] = new_detail
                vague_fix_count += 1
        for field in ["description", "catalyst", "risk"]:
            val = hp.get(field, "")
            if val:
                new_val, fixed = try_resolve_vague(val, sname, all_data)
                if fixed:
